...
// const SpecialEvents = ({ addSpecialEvent }) => {
//   const [occasion, setOccasion] = useState("");
//   const [reminder, setReminder] = useState("");
//   const [selectedDate, setSelectedDate] = useState(new Date());
//   const [publicHolidays, setPublicHolidays] = useState([]);
...
//       const specialEventsData = response.data.filter(event => event.is_special_event);
...
//     if (occasion && reminder && duration) { // Make sure duration is not blank
//       try {
...
//         setSpecialEvents([...specialEvents, response.data]);
//         setOccasion("");
//         setReminder("");
